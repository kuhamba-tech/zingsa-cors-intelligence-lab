...
import { loadIndex } from './corsDataIngest.js';
...
function seededStatus(stationId, index, hourSeed) {
 ...
 * Uses archive recency when available; hourly seed only for gaps.
 */
export function buildStationHealthRows(stations, { now = new Date(), archives } = {}) {
  const archiveMap = buildArchiveMap(archives);
  const hourSeed = seedFor(now.toISOString().slice(0, 13));
  ...
  const rows = stations.map((station, index) => {
    ...
    seedFallback += 1;
    const seeded = seededStatus(stationId, index, hourSeed);
    return {
      ...station,
      ...seeded,
  ...
  const mode = telemetry_source === 'hourly-seed' ? 'simulated' : 'blended';
