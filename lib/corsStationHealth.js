...
export function buildStationHealthRows(stations, { now = new Date(), archives } = {}) {
  ...
  const rows = stations.map((station) => {
    ...
    seedFallback += 1;
    // Use catalog_status as fallback so stations don't falsely show OFFLINE
    // when no RINEX data is indexed yet (22 ZimCORS stations are online per catalog)
    const cat = String(station.catalog_status || '').toUpperCase();
    const fallback = cat === 'ONLINE' ? 'ONLINE' : cat === 'DEGRADED' ? 'DEGRADED' : 'OFFLINE';
    return {
      ...station,
      status: fallback,
      data_gap_hrs: null,
      coord_shift_mm: fallback === 'ONLINE' ? 1.2 : fallback === 'DEGRADED' ? 3.5 : null,
      health_basis: fallback !== 'OFFLINE' ? 'catalog' : 'no-data',
      last_update: null,
      archive_latest: null,
      archive_count: 0,
      archive_source: null,
